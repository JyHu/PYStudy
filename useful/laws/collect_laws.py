		...
		while True:
			...
			if len(law_list) > 0: 		# 如果抓取到的法律列表有内容，就抓取每部法律下的具体内容
				for res in law_list:	# 遍历去抓取具体内容
					print "已抓取到[%03d]部法律内容，正在抓取 《%s》 中的第[%03d]部法律 《%s》 的内容" % (total_count, folderName, cur_count, res[1].strip())
					catch_law_detail("%s/%s/%s" % (baseURL, law, res[0]), "%s/%d-%s.txt" % (folderName, cur_count + 1, res[1].strip()))
